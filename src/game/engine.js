 ...
 * - Register focused runtime systems from src/game/systems.
 ...
 *   for the engine runtime. Script order in src/game/index.html is therefore part of
 *   the architecture.
 ...
 *
 * Related files:
 * - src/game/systems/*.js contain focused runtime behavior.
 * - src/engine/dataLoader.js wraps IPC for data/save IO.
 * - src/engine/dataRegistry.js compiles base content and mods.
 * - src/game/ui/ui.js provides the rendering facade used by Engine.
 */
const Engine = {
	...
	init: async function () {
		...
		try {
			...
			if (rawData.storyConfig) {
				Object.assign(Config, rawData.storyConfig);
			}

			document.documentElement.style.fontSize = `${16 * (Config.font_scale || 1)}px`;
